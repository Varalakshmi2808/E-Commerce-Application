...
const Search = () => {
    ...
    useEffect(()=>{
        axios.get(`http://localhost:5000/srch/${obj.usercon.srch}`).then((res)=>{
          setData(res.data)
          console.log(res.data)
        })
    },[obj.usercon.srch])
    let del=(item)=>{
      axios.delete(`http://localhost:5000/delprod/${item._id}/${item.pimg}`).then((res)=>{
        setF((f)=>(!f))
    ...
    let addcart=(item)=>{
      ...
      axios.post("http://localhost:5000/addcart",{...item,"qty":1,"uid":obj.usercon._id,"pid":item._id}).then((res)=>{
        alert(res.data.msg)
      })
    }
    }
